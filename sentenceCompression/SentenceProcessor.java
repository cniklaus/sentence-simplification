...
public class SentenceProcessor {
	...
	public static boolean pruneContextSentences(CoreContextSentence sentence) {
		...
		//	}
			...
			if (n < sentence.getContext().size()) {	
				isPrunedAccording = GerundClauseExtractor.extractAccording(sentence, currentContext.get(n), false, n);
			}
			if (n < sentence.getContext().size()) {	
				isPrunedGerundAfterWhile = GerundClauseExtractor.extractGerundAfterWhile(sentence, currentContext.get(n), false, n);
			}
			//if (n < sentence.getContext().size()) {	
				//isPrunedGerundAfterComma = GerundClauseExtractor.extractGerundAfterComma(sentence, currentContext.get(n), false, n);
			//}
			if (n < sentence.getContext().size()) {	
				isPrunedByGerund = GerundClauseExtractor.extractByGerund(sentence, currentContext.get(n), false, n);
			}
			//}
			if (n < sentence.getContext().size()) {	
			...
			if (n < sentence.getContext().size()) {	
				isPrunedADJPappositives = AppositivePhraseExtractor.extractAdjectivePhraseAppositives(sentence, currentContext.get(n), false, n);
			}
			if (n < sentence.getContext().size()) {	
				isPrunedNPappositives = AppositivePhraseExtractor.extractNounPhraseAppositives(sentence, currentContext.get(n), false, n);
			}
			if (n < sentence.getContext().size()) {	
				isPrunedVPappositives = AppositivePhraseExtractor.extractVerbPhraseAppositives(sentence, currentContext.get(n), false, n);
			}
			if (n < sentence.getContext().size()) {
				isPrunedADVPappositives = AppositivePhraseExtractor.extractAdverbialPhraseAppositives(sentence, currentContext.get(n), false, n);
			}
			if (n < sentence.getContext().size()) {	
				isPrunedNPPappositives = AppositivePhraseExtractor.extractProperNounPhraseAppositives(sentence, currentContext.get(n), false, n);
			}
			if (n < sentence.getContext().size()) {	
				isPrunedGerundAfterNNP = GerundClauseExtractor.extractGerundAfterNNP(sentence, currentContext.get(n), false, n);
			}
			if (n < sentence.getContext().size()) {	
				isPrunedIncluding = GerundClauseExtractor.extractIncluding(sentence, currentContext.get(n), false, n);
			}
			//if (n < sentence.getContext().size()) {	
				//isPrunedGerundSentenceStart = GerundClauseExtractor.extractGerundSentenceStart(sentence, currentContext.get(n), false, n);
			//}
			if (n < sentence.getContext().size()) {	
				isPrunedProperNouns = ProperNounsExtractor.extractAdjectivesNounsNNPs(sentence, currentContext.get(n), false, n);
			}
			if (n < sentence.getContext().size()) {	
				isPrunedColon = Punctuation.splitAtColon(sentence, currentContext.get(n), false, n);
			}
			if (n < sentence.getContext().size()) {	
				isPrunedParentheses = Punctuation.parentheses(sentence, currentContext.get(n), false, n);
			}
			if (n < sentence.getContext().size()) {	
